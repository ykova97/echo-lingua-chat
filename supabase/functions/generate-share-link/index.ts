...
serve(async (req) => {
  ...
  try {
    ...
    // Build the share URL using PUBLIC_APP_URL environment variable
    const baseUrl = Deno.env.get("PUBLIC_APP_URL") || "https://lynk-chat.com";
    const shareUrl = `${baseUrl.replace(/\/$/, "")}/guest/${token}`;
