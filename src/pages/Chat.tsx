...
export default function Chat() {
  ...
  async function loadMessagesPage(before?: string) {
    if (!chatId) return { messages: [], nextCursor: null as string | null };
  ...
  useEffect(() => {
    ...
    // Smoothly ensure last message is visible
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const { error } = await supabase.from("messages").insert({
        ...
        source_language: "en", // TODO: set from profile preferred_language if available
      });
      if (error) throw error;
    ...
    <div className="chat-shell">
      ...
      <header className="sticky-header border-b bg-background/80 supports-[backdrop-filter]:bg-background/60">
        <div className="px-4 py-3 flex items-center gap-3">
          ...
          <h1 className="text-lg font-semibold truncate">Chat</h1>
        </div>
      </header>
      ...
      {/* Sticky Input (lifted above keyboard) */}
      <div className="sticky-footer kb-safe border-t bg-background">
        <div className="p-3 flex gap-2">
          <Input
            ...
            onFocus={() => {
              // make sure the last message is visible when keyboard opens
              setTimeout(() => bottomSentinelRef.current?.scrollIntoView({ block: "end", behavior: "smooth" }), 0);
            }}
          />
